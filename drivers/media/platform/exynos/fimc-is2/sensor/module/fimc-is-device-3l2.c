	...
	struct exynos_platform_fimc_is_module *pdata)
{
	...
	if (gpio_is_valid(gpio_af_pwr_en)) {  /* VDDAF_2.8V_CAM */
		SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_af_pwr_en, NULL, PIN_OUTPUT, 1, 3000);
	} else {
		SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_none, "VDDAF_2.8V_CAM", PIN_REGULATOR, 1, 3000);
	}
#endif
	...
	if (gpio_is_valid(gpio_ois_reset)) {  /* OIS_RESET */
		SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_ois_reset, NULL, PIN_OUTPUT, 1, 3000);
	}
	SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_none, "pin", PIN_FUNCTION, 0, 6500);
